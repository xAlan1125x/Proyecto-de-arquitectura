...
.DATA                 ; Inicio del segmento de datos
    num     DW ?                 ; Variable 'num' (word 16 bits) inicializada a 5
    ...
    bufnum1 db 6, ?, 6 dup(?)    ; buffer para leer hasta 6 caracteres desde teclado
...
START:                ; Etiqueta de entrada del programa
    ...
    ; --- Leer n?mero desde teclado ---
    mov ah, 0Ah        ; funci?n DOS: buffered input
    mov dx, offset bufnum1
    int 21h

    ; --- Convertir ASCII a n?mero (solo 1 d?gito) ---
    mov al, bufnum1+2   ; primer car?cter ingresado
    sub al, '0'          ; convertir de ASCII a valor num?rico
    mov ah, 0            ; limpiar parte alta de AX
    mov [num], ax        ; guardar en variable num
